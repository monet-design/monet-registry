...
async function updateTestimonialCategories() {
  ...
  for (const filePath of metadataFiles) {
    ...
    // Check if name contains 'testimonial' (case insensitive)
    if (name && name.toLowerCase().includes('testimonial')) {
      // Only update if category is not already 'testimonial'
      if (currentCategory !== 'testimonial') {
        const newContent = updateCategoryInYaml(content, 'testimonial');
        ...
        console.log(`Updated: ${name} (${currentCategory || 'undefined'} -> testimonial)`);
      }
    }
  }
  ...
  if (updatedFiles.length > 0) {
    ...
    updatedFiles.forEach(f => {
      console.log(`  - ${f.name}: ${f.oldCategory || 'undefined'} -> testimonial`);
    });
  }
}

updateTestimonialCategories().catch(console.error);
